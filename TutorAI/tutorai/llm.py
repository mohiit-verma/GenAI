...
from config import DEFAULT_LLM_CATALOG
...
class LLMManager:
    """Load and run open-source LLMs via HuggingFace transformers `pipeline`."""
    ...
    def load(self, name: str, gen_kwargs: Optional[Dict[str, Any]] = None):
        ...
        # sensible generation defaults; override per call if you want
        self.gen_defaults = {
            "max_new_tokens": 384,
            "temperature": 0.2,
            "top_p": 0.9,
    ...
    def generate(self, prompt: str, **overrides) -> str:
        if self.pipe is None:
            raise RuntimeError("LLM not loaded. Call LLMManager.load(name) first.")
